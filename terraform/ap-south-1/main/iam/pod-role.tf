...
locals {
  customer_policies = distinct(flatten([
    for name, config in local.pod_roles : [
      ...
      policy.type == "customer" && config.enable == true ? [{ name = policy.name, namespace = config.namespace, env = config.env }] : []
    ]
  ]))
  ...
  managed_policies = distinct(flatten([
    for name, config in local.pod_roles : [
      for policy in config.policy :
      policy.type == "managed" && config.enable == true ? [{ name = policy.name, namespace = config.namespace, env = config.env }] : []
    ]
  ]))
...
resource "aws_iam_policy" "pod_policy" {
  for_each = {
    ...
    "${policy.env}-${policy.namespace}-${policy.name}" => policy
  }

  name        = "pod-policy-${each.value.env}-${each.value.namespace}-${each.value.name}"
  path        = "/"
  description = "Policy for env ${each.value.env} - ${each.value.name} service in namespace ${each.value.namespace}"
...
module "eks_role_pod" {
  ...
  name               = "pod-role-${each.value.env}-${each.value.namespace}-${each.key}"
  oidc_provider_urls = [local.oidc_providers[each.value.env]]
  ...
  policies = {
    ...
    aws_iam_policy.pod_policy["${each.value.env}-${each.value.namespace}-${policy.name}"].arn :
